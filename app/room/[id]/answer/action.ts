...
// 回答者が決定しているか確認
export async function isCheckAnswer(
    roomId: string,
) {
    try {
        const { data, error } =
...
// 回答者の登録
export async function setdbAnswer(
    roomId: string,
    userId: string,
) {
    try {
...
export async function getDrawingsByRoom(
    roomId: string,
) {
    try {
        const { data, error } =
...
export async function checkAnswerRole(
    roomId: string,
    userId: string,
) {
    try {
...
// お題の正誤判定のため複数パターンを取得
export async function getThemePatternByRoomId(
    roomId: string,
) {
    const id = roomId;
    let themeId: string = "";
    ...
    try {
        ...
        if (error) {
            console.error(
                "Failed to fetch current theme id:",
                error,
            );
            return {
                success: false,
    ...
    try {
        ...
        if (error) {
            console.error(
                "Failed to fetch theme patterns:",
                error,
            );
            return {
                success: false,
 ...
 * 回答の登録
 */
export async function setdbAnswerInput(
    roomId: string,
    answer: string,
) {
    try {
 ...
 * 回答結果の記録
 */
export async function setdbAnswerResult(
    roomId: string,
    result: string,
) {
    try {
