...
export async function getRoomByPageSearch(
    page: number,
    pageSize: number,
    searchTerm: string,
) {
...
export async function getRoomsByUserId(
    ...
    try {
        ...
        console.log(
            "Rooms fetched by user ID:",
            data,
        );
        if (error) {
            console.error(
                "Failed to fetch rooms by user ID:",
                error,
            );
            return {
                success: false,
...
// ルームを作成
export async function createRoomByUsername(
    createRoomData: CreateRoom,
) {
    const sanitizedRoomName =
        createRoomData.roomName;
    const sanitizedUsername =
        createRoomData.username;
    const sanitizedLevel =
        createRoomData.level;
    const sanitizedGenre =
        createRoomData.genre;
    const userId =
        createRoomData.userId || null;
    let randomTheme;
...
export async function getRoomByShortId(
    ...
    try {
        ...
        if (error) {
            console.error(
                "Failed to fetch room by short ID:",
                error,
            );
            return {
                success: false,
