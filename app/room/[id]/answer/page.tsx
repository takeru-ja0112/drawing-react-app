...
import { getDrawingsByRoom , getTheme} from '@/app/room/[id]/answer/action';

export default async function Page({ params }: { params: Promise<{ id: string }> }) {
    const { id: roomId } = await params;
    const result = await getDrawingsByRoom(roomId);
    const themeResult = await getTheme(roomId);
    ...
    return <AnswerPage roomId={roomId} drawings={drawings} theme={theme} />;
}